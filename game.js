...
document.getElementById("rebetBtn").addEventListener("click", ()=>{
    ...
    if(bankroll >= lastBet){
        bankroll -= lastBet;
        bet += lastBet;
        betHistory = [...Array(Math.floor(lastBet/50)).fill(50)]; // esempio per ripristino
        updateUI();
    } else alert("Saldo insufficiente per ripetere la puntata");
});

// ----------------- DOUBLE BET -----------------
document.getElementById("doubleBetBtn").addEventListener("click", ()=>{
    if(gameOver) return;
    if(bet === 0) return alert("Prima devi puntare!");
    if(bankroll >= bet){
        bankroll -= bet;
        bet *= 2;
        betHistory.push(...betHistory);
        updateUI();
    } else alert("Saldo insufficiente per raddoppiare");
});
